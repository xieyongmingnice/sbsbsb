...
/**
 * @author xym
 * @description 回复去黑关键字管理控制器
 * @date 2018/5/9 14:36
 */
@RestController
@RequestMapping("/rmblacklistkeyword")
public class RmBlacklistKeywordController {

    @Autowired
    RedisUtil redisUtil;

    @Autowired
    RmBlacklistKeywordService keywordService;
    ...
    @ResponseBody
    public Result<String> addKeyword(RmBlacklistKeywordModel model){
    ...
    @ResponseBody
    public Result<String> deleteKeyword(RmBlacklistKeywordModel model){
        Result result = checkLogin(new Result(), model.getSessionId());
        if (result.getMessage() != null) {
            return result;
        }
    ...
    public Result<List<RmBlacklistKeyword>> getKeywordList(RmBlacklistKeywordModel model){
        ...
        List<RmBlacklistKeyword> list = keywordService.selectKeywordList(model);
    ...
    @ResponseBody
    public Result<String> batchDeleteKeyword(RmBlacklistKeywordModel model){
        Result result = checkLogin(new Result(),model.getSessionId());
        if(result.getMessage() != null ){
            return result;
        }
    ...
    public Result<String> excelAddKeyword(HttpServletRequest request,String sessionId) throws Exception{
        ...
        List<RmBlacklistKeyword> list = Lists.newArrayList();
